...
#mapping velocity range into motor commands range: velocity: -1 - 3 m/s  motor_commands: -1 - 1:
 ...
 #mapping angular value from -1.8 rad per second to 1.8 rad per second to -1 to 1 motor values:
    ...
    if (x!=0 and y!=0 and z==0): #or (x!=0 and y==0 and z==0) or (x==0 and y!=0 and z==0)):
        motor_commands = numpy.array([motor_value , motor_value , bow_thruster_value/angular_factor , servo_angle_command , servo_angle_command, 0])
    #second state - 3 axis movement:
            ...
            if (((bow_thruster_value / angular_factor) + angular_direction_factor * angular_value) >= 1) :
                ...
                ((bow_thruster_value / angular_factor) + angular_direction_factor * angular_value) , servo_angle_command , servo_angle_command, 0])
    ...
    # 4th state - moving forward
    elif (x!=0 and y==0 and z==0):
        motor_commands = numpy.array([motor_value , motor_value , 0  , servo_angle_command , servo_angle_command, 0])
